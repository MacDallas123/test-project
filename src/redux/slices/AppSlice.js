  ...
  invoices: [
    {
      ...
      items: [
        { description: "Pizza Margherita x2", quantity: 2, unitPrice: 10.99, total: 21.98 },
        { description: "Poulet Yassa x1", quantity: 1, unitPrice: 8.5, total: 8.5 }
      ],
      subtotal: 30.48,
      tax: 6.10, // 20% TVA
      total: 36.58,
  ...
  quotes: [
    {
      ...
      items: [
        { description: "Formule Traiteur Standard", quantity: 1, unitPrice: 500, total: 500 }
      ],
      subtotal: 500,
      tax: 100,
      total: 600,
...
const appSlice = createSlice({
  ...
  reducers: {
    ...
    login: (state, action) => {
      ...
      const foundUser = fakeData.users.find((user) => user.email === email);
    ...
    register: (state, action) => {
      const { name, email, phone, type, address, password } = action.payload || {};
    ...
    updateFood: (state, action) => {
      ...
      if (index !== -1) {
        ...
        const fakeDataIndex = fakeData.foods.findIndex(food => food.id === id);
        if (fakeDataIndex !== -1) {
          fakeData.foods[fakeDataIndex] = { ...fakeData.foods[fakeDataIndex], ...updates };
        }
      }
    },
    ...
    deleteFood: (state, action) => {
      ...
      fakeData.foods = fakeData.foods.filter(food => food.id !== action.payload);
    },
    ...
    deleteJobOffer: (state, action) => {
      state.jobOffers = state.jobOffers.filter(job => job.id !== action.payload);
      fakeData.jobOffers = fakeData.jobOffers.filter(job => job.id !== action.payload);
    },
    ...
    updateInvoiceStatus: (state, action) => {
      ...
      if (index !== -1) {
        ...
        if (status === "paid") {
          state.invoices[index].paymentDate = new Date().toISOString().split('T')[0];
        }
      }
    },
    ...
    createQuote: (state, action) => {
      ...
      const newQuote = {
        ...
        issueDate: new Date().toISOString().split('T')[0],
        expiryDate: new Date(Date.now() + (validityDays || 30) * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        notes,
...
const generateInvoiceNumber = (state) => {
  ...
  const sequence = state.invoices.filter(inv => inv.invoiceNumber.includes(year.toString())).length + 1;
  return `FA-${year}-${sequence.toString().padStart(4, '0')}`;
};

const generateQuoteNumber = (state) => {
  const year = new Date().getFullYear();
  const sequence = state.quotes.filter(q => q.quoteNumber.includes(year.toString())).length + 1;
...
export const selectCartTotal = (state) => 
  state.app.cart.reduce((total, item) => total + (item.price * (item.quantity || 1)), 0);
